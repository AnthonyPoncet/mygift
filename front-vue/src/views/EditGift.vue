...
import type {
  ...
  ShowingFriendGift,
  Wishlist,
} from "@/components/helpers/common_json.ts";
...
async function getCategories() {
  ...
  let endpoint;
  if (route.name == "mywishlisteditgift") {
    endpoint = "/wishlist";
  } else {
...
async function getGift() {
  await getCategories();
  if (route.name == "mywishlisteditgift") {
    const response = await make_authorized_request(router, `/wishlist/gifts/${route.params.id}`);
    if (response !== null) {
      const gift: EditGift = await response.json();
      name.value = gift.name;
      description.value = gift.description;
      price.value = gift.price;
      whereToBuy.value = gift.where_to_buy;
      category.value = gift.category;
      if (gift.picture != null) {
        const response = await make_authorized_request(router, `/files/${gift.picture}`);
        if (response !== null) {
    ...
    const response = await make_authorized_request(
      router,
      `/wishlist/friend/${friendId.value}/gifts/${route.params.id}`,
    );
    if (response !== null) {
      const gift: ShowingFriendGift = await response.json();
      name.value = gift.gift_name;
      description.value = gift.description;
      price.value = gift.price;
      whereToBuy.value = gift.where_to_buy;
      category.value = categories.value.find((c) => c.name === gift.category_name)!.id;
      if (gift.picture != null) {
        const response = await make_authorized_request(router, `/files/${gift.picture}`);
        if (response !== null) {
          const blob = await response.blob();
...
async function add(e: Event) {
  ...
  let endpoint;
  if (route.name == "mywishlisteditgift") {
    endpoint = `/wishlist/categories/${category.value}/gifts/${route.params.id}`;
  } else {
    const friendId = await getFriendDetails(route.params.name as string);
    endpoint = `/wishlist/friend/${friendId}/categories/${category.value}/gifts/${route.params.id}`;
  }
...
<template>
  <div class="main-container grid grid-cols-1 justify-items-center">
    <h1 class="mb-5 text-center text-4xl font-normal">
      {{
        route.name == "mywishlisteditgift"
          ? useLanguageStore().language.messages.gift_modal__updateGiftTitle
          : useLanguageStore().language.messages.gift_modal__updateSecretGiftTitle
      }}
    </h1>
