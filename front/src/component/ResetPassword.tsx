...
function isUuidValid(uuid: string, setStatus: any, appDispatch: any) {
    const request = async () => {
        const response = await fetch(url + '/passwords/reset/' + uuid, { method: 'get' });
...
function handleSubmit(uuid: string, username: string, password: string, setStatus: any, appDispatch: any) {
    const request = async () => {
    ...
    if (status === "loading") {
        ...
        <div className="auth-form">
            ...
            <div className="auth-form-body">
                ...
                <br/>
                <button className="btn btn-primary btn-large" onClick={() => handleSubmit(uuid, username, password, setStatus, appDispatch)}>Change</button>
            </div>
        </div>
        );
